...
if [ \"$DOMAIN\" ]; then
    ...
    case \"$SEND_EMAIL\" in
        'Yes')
            ...
            MX='Yes'
            SPF='Yes'
            DKIM='Yes'  # Not yet implemented
            DMARC='Yes' # Not yet implemented

            # rDNS
            # Install dig now so apt-get doesn't hold lock
            # on dpkg when set_rdns() forks into the background
            if ! dig; then
    ...
    # Define the DNS records to be created
    ...
    # is the API token defined?
    if [ \"$TOKEN_PASSWORD\" ]; then
        set_dns 'dns_records' &
    fi
else
    # Set FQDN to default rDNS if no domain is in use
